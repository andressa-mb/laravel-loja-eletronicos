...
@section('content')
    <div class="row form-create-bg">
        ...
        <form action="{{route('update-user', $user)}}" method="POST" class="m-auto w-50 form-create">
            ...
            <div class="form-group">
                <label for="email" class="font-form">{{__('messages.email')}}:</label>
                <input type="email" id="email" name="email" class="form-control" value="{{$user->email}}">
            </div>
            <div class="form-group">
                <label class="font-form">{{__('messages.alterar_permissoes')}}</label>
            ...
            <div>
                ...
                <a type="button" class="btn btn-primary" href="{{route('users-list')}}">{{__('messages.voltar')}}</a>
            </div>
        </form>
    </div>
@endsection
