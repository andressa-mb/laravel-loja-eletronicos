...
@section('content')
    <div class="row">
        <div class="col-12 my-5 text-right">
            <a href="{{route('index-buyer')}}" class="btn btn-info">{{__('messages.voltar')}}</a>
        </div>
        <div id="detalhes-produto" class="col-12 row d-flex justify-content-center align-items-center">
                    ...
                    <div class="mt-3">
                        ...
                        @if ($hasWish)
                            <a href="{{route('remove-wish', $hasWish->id)}}" class="d-flex text-decoration-none text-danger font-weight-bold">
                                ...
                                <span class="ml-3">{{__('messages.remover_lista_desejo')}}</span>
                            </a>
                        @else
                            <a href="{{route('add-wish', $product)}}" class="d-flex text-decoration-none text-danger font-weight-bold">
                                ...
                                <span class="ml-3">{{__('messages.adicionar_lista_desejo')}}</span>
                            </a>
                        @endif
                    </div>
                @endif
            ...
            <form class="col-6" action="{{route('selling-product-info-client', $product)}}" id="info">
                ...
                <div class="form-group col-12">
                    <label for="description" class="font-form"><strong>{{__('messages.descricao')}}:</strong></label>
                ...
                @if ($product->quantity > 0)
                    <div class="form-group col-12">
                        <label for="quantity" class="font-form"><strong>{{__('messages.quantidade')}}:</strong></label>
                ...
                {{-- CATEGORIAS --}}
                <div class="form-group col-12">
                    <label for="category" class="font-form"><strong>{{__('messages.categoria')}}:</strong></label>
                    @if ($product->categories->isEmpty())
                        <input type="text" name="category" class="form-control-plaintext" id="category" value="{{__('messages.sem_cat_atrib')}}" readonly>
                    @else
                ...
                {{-- PREÇO --}}
                <div class="form-group col-12">
                    <label for="price" class="font-form"><strong>{{__('messages.valor_produto')}}:</strong></label>
                ...
                @if ($product->hasDiscount && $product->isDiscountActive())
                    <div class="form-group col-12">
                        <label for="discount_type" class="font-form"><strong>{{__('messages.desconto')}}:</strong></label>
                    ...
                    <div class="form-group col-12">
                        <label for="total" class="font-form"><strong>{{__('messages.total')}}:</strong></label>
                ...
                @if (Auth::user())
                    @if ($product->quantity <= 0)
                        <button type="submit" class="btn btn-success mt-3" disabled data-toggle="tooltip" data-placement="top" title="Produto indisponível">{{__('messages.comprar')}}</button>
                    @else
                        <button type="submit" class="btn btn-success mt-3" name="action" value="comprar">{{__('messages.comprar')}}</button>
                        <button type="submit" class="btn btn-primary mt-3" name="action" value="carrinho">{{__('messages.carrinho')}}</button>
                    @endif
                @else
